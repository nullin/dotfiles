...
|----------|---------|---------|
| `machine_type` | `"work"` or `"personal"` - drives conditional content | settings.json, configuration.nix, zshrc, cursor settings |
| `name` | Full name for git identity | gitconfig |
| `email` | Email for git identity | gitconfig |
| `github_username` | GitHub username | gh hosts.yml |
| `goprivate` | Go private module path (empty on personal) | zshrc, cursor settings |
| `username` | macOS username | configuration.nix, flake.nix |
| `corporate_path` | Corporate bin directory (empty on personal) | configuration.nix |

On a new Mac, run `chezmoi init nullin` to be prompted for these values. The hostname is auto-detected via `.chezmoi.hostname`.

## Templated Files (7 files)
...
| `dot_config/nix/configuration.nix.tmpl` | `{{ .corporate_path }}`, `{{ .username }}`, `{{ .machine_type }}` |
| `dot_config/nix/flake.nix.tmpl` | `{{ .chezmoi.hostname }}`, `{{ .username }}` |
| `dot_config/cursor/settings.json.tmpl` | `{{ .goprivate }}` |
| `dot_config/gh/private_hosts.yml.tmpl` | `{{ .github_username }}` |
| `dot_claude/settings.json.tmpl` | `{{ .machine_type }}` |
| `dot_config/gh/private_hosts.yml.tmpl` | `{{ .github_username }}` |
