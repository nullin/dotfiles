...
### Safety First
- **CoreWeave clusters:** Load the `cw-teleport` skill first. Never use the default kubeconfig (`~/.kube/config`) - always set `KUBECONFIG` explicitly per command to avoid targeting the wrong cluster.
