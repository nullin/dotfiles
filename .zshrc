...
export FZF_CTRL_T_OPTS="--preview '[ -d {} ] && tree -C {} || cat --color=always --style=numbers {}'"
...
mc() {
  ...
  if [[ -z "$name" ]]
  then
    pushd "$CLAUDE_WORK_DIR" > /dev/null || return
    claude --dangerously-skip-permissions --verbose
    popd > /dev/null
    return
  fi
  local dir="$CLAUDE_WORK_DIR/$name"
  local created=0
  if [[ ! -d "$dir" ]]
  then
    mkdir -p "$dir" && created=1
  fi
  ...
  local -a transcripts
  transcripts=("$proj_dir"/*.jsonl(N))
  if (( ${#transcripts} > 0 )) && grep -lq "\"customTitle\":\"$name\"" "${transcripts[@]}"
  then
    claude --dangerously-skip-permissions --verbose --resume "$name"
  else
    claude --dangerously-skip-permissions --verbose --name "$name"
  fi
  popd > /dev/null
}
