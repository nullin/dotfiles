...
Returns all dashboards in the organization.
...
### Retrieve a Dashboard
    ...
    GET /organizations/{org}/dashboards/{dashboard_id}/

Returns full dashboard with all widgets, queries, conditions, and layout.
...
### Update a Dashboard
    ...
    PUT /organizations/{org}/dashboards/{dashboard_id}/

Replaces the entire dashboard. Send the full payload (title, widgets, filters, period).
...
| widgets | array  | Full array of widget objects (see Widget Object below)     |
| filters | object | Dashboard-level filters                                    |
| period  | string | Time period: `24h`, `7d`, `14d`, `30d`, etc.              |

**Important:** Widget IDs are regenerated on every PUT. Never reference widget IDs from a previous response - they will not match. Match widgets by `title` instead.

    curl -s -X PUT -H "Authorization: Bearer $TOKEN" \
      -H "Content-Type: application/json" \
      -d '{"title":"My Dashboard","widgets":[...],"filters":{},"period":"7d"}' \
      "https://sentry.io/api/0/organizations/$SENTRY_ORG/dashboards/$DASHBOARD_ID/"

### Widget Object (key fields)

| Field       | Type   | Description                                              |
|-------------|--------|----------------------------------------------------------|
| id          | string | Widget ID (regenerated on every PUT - do not cache)      |
| title       | string | Widget display title (stable, use for matching)          |
| displayType | string | `table`, `big_number`, `area`, `line`, `bar`             |
| widgetType  | string | `spans` (span data) or `issue` (issue data)              |
| queries     | array  | Array of query objects (see below)                       |
| layout      | object | Position and size: `{x, y, w, h, minH}`                 |
| interval    | string | Data interval: `5m`, `1h`, etc.                          |
| limit       | int    | Max results (null for default)                           |

### Widget Query Object

| Field          | Type   | Description                                         |
|----------------|--------|-----------------------------------------------------|
| id             | string | Query ID (regenerated on PUT)                       |
| conditions     | string | Search filter string (span or issue syntax)         |
| fields         | array  | Columns to display                                  |
| aggregates     | array  | Aggregate functions                                 |
| columns        | array  | Group-by columns                                    |
| fieldAliases   | array  | Display names for fields                            |
...
### List Available Fields
    ...
    GET /organizations/{org}/spans/fields/

Query parameters:

| Parameter   | Type   | Description                                          |
|-------------|--------|------------------------------------------------------|
| statsPeriod | string | Time range for field discovery: `24h`, `7d`, etc.    |

    curl -s -H "Authorization: Bearer $TOKEN" \
      "https://sentry.io/api/0/organizations/$SENTRY_ORG/spans/fields/?statsPeriod=7d"

Returns array of `{key, name}` objects. Common fields: `user`, `user.username`, `cli.command`, `transaction`, `release`, `component.name`.

### Get Field Values

    GET /organizations/{org}/spans/fields/{field}/values/

Query parameters:

| Parameter   | Type   | Description                                          |
|-------------|--------|------------------------------------------------------|
| statsPeriod | string | Time range: `24h`, `7d`, etc.                        |

    curl -s -H "Authorization: Bearer $TOKEN" \
      "https://sentry.io/api/0/organizations/$SENTRY_ORG/spans/fields/user/values/?statsPeriod=7d"
...
| name      | string | Display value (e.g., `username:root`)                  |
| value     | string | Raw value                                              |
| query     | string | Correct filter syntax for this value (use this)        |

The `query` field is the most useful - it gives the exact filter syntax to use in widget conditions. Example: `user.username:"root"`.
