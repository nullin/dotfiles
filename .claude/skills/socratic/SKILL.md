---
name: socratic
description: Exhaustive co-interview between Claude and Codex for thorough analysis from every angle. Use for "get socratic", "debate this", "deep dive", "challenge all assumptions", or rigorous exploration before major decisions.
...
Start the Codex thread with `mcp__codex-cli__codex`. Your opening should:
...
Continue with `mcp__codex-cli__codex` (pass the `sessionId` returned by the first call). Each round must:
...
**Question Categories** (inspiration - pull in the ones that fit the topic, not a checklist to tick):
...
When a round stops surfacing genuinely new angles, check with Codex whether to continue:

> "What angles remain unexplored? Are there questions we should be asking that we haven't? Or have we gone deep enough?"

Continue while rounds keep producing new insight; wind down when they don't. Depth matters more than speed, but stop when the topic is genuinely worked through rather than padding rounds. If the user set a round cap, respect it.
...
- A genuinely relevant angle hasn't been explored yet
...
- The relevant angles have been explored
- Neither side can generate genuinely new angles
- Any remaining uncertainty requires user input to resolve

Don't stop prematurely just because:
- You've reached agreement (agreement can mean you're both wrong - probe it once)
- It feels "good enough" (push past the first comfortable stopping point)
- Only the obvious points have been covered (those are the easy ones)

But once rounds stop yielding new insight, converge - more rounds past that point add tokens, not understanding.
