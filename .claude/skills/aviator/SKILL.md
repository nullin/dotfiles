...
description: Stacked PR workflows via Aviator CLI (`av`) - create stacks, sync branches in a stack, manage multi-part features with dependent branches. Use for "stacked PR", "av stack", "aviator", or features that should split into incremental PRs.
---
...
## Use av to Create Branches

Create stack branches with `av branch`, not raw `git checkout -b` or `git branch` - branches created outside `av` are not tracked in av's metadata, which means:
...
Lesson: use `av branch` so `av restack` can handle this automatically.
...
- Create branches with `av branch`, not raw git
