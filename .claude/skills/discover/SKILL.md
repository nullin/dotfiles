---
name: discover
description: Deep codebase investigation using the Explore subagent. Use for "how does X work in this codebase", "trace the flow", "find where X is implemented", or architecture questions. Use repo-explore instead for external GitHub repos.
allowed-tools:
  - Read
  - Glob
  - Grep
...
Delegating the search to Explore subagents is the core of this skill. Spawn at least one; reach for direct Grep/Read only to verify a specific finding in Steps 4-5, not as a substitute for the initial Explore pass.

- Use the Task tool with `subagent_type: "Explore"` and thoroughness: "very thorough".
- Scale the number of queries to the question: a focused single-file question needs one query; a broad architecture question warrants up to three. Launch independent queries in the same turn so they run in parallel:
