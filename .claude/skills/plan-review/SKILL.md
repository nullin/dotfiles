...
Always run this Codex pass - don't substitute your own review of the plan, since you
share the context that produced it. If Codex is unavailable, say so and run the same
gap analysis yourself against the checklist below.
...
in production. Be specific. Surface gaps even if you're uncertain whether they apply -
mark each as likely / possible / theoretical so the reader can filter. Don't fabricate
concerns for decisions that are clearly fine.
