...
This skill's value comes from independent specialist reviewers. Always create the team and spawn the selected specialists (Step 3) - orchestrate the review rather than doing a single-pass review yourself.
...
Task(
  ...
  prompt: "RISK LANE: L1\n\nSELF-CRITIQUE REQUIREMENT:\nThis is an L1/L2 review. After your specialist review, stress-test your findings through self-critique before sending them. Follow your Self-Critique protocol, but annotate each finding with a confidence level (high/medium/low) instead of dropping uncertain ones - report it and let synthesis weigh it. Only withdraw a finding if self-critique actively shows it is wrong, not merely uncertain.\n(For L0 reviews, replace the above with: SELF-CRITIQUE: Not required for L0. Send findings directly.)\n\nPR CONTEXT:\n<PR description and context>\n\nDIFF TO REVIEW:\n<the full diff>\n\nYour task has been created as Task #N. Update it to in_progress when you start, and mark it completed when done sending findings."
)
```
...
Each agent's prompt must include the full diff text - agents can't see the diff unless you include it in their prompt.
...
5. **Integrate**: For each finding, note whether it was:
   ...
   - **Withdrawn**: the challenge actively showed the finding was wrong (not merely that the original agent backed down)
