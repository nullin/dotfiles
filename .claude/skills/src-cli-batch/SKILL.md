...
**Before deleting:** Check changeset stats. Deleting a batch change does NOT close open PRs on GitHub - those remain open. If you want to clean them up first, use the UI's bulk "Close changesets" action before deleting. Confirm with the user before deleting a batch change that has open (non-draft) changesets.
