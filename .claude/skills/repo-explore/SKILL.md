---
name: repo-explore
description: Clones and explores external GitHub repos to understand how libraries, frameworks, or dependencies work. Use when the user shares a github.com URL, asks "how does X library work", or wants to look at source for a dependency. Auto-checks out the matching version tag when the repo is a dependency in the current project.
context: fork
allowed-tools:
  - Bash(git:*)
  - Bash(ls:*)
...
For non-trivial questions about how the repo works, delegate to the Explore subagent (`subagent_type=Explore`) rather than browsing files yourself, and spawn parallel Explore agents for independent questions. The Explore agent is optimized for:
...
For substantial answers, end with a **"Key Files for Further Exploration"** section:
